...
const createToDo = async (data, user) => {
    ...
    const checkResult = await Mongo.findData('todos', {$and : [{ title : title.trim() }, { username : user.username }, {deleteStatus : 0}]})
...
const completeMarkOperation = async(data, user) => {
    ...
    if(updateTodo){
        let message = ''
        completeStatus == 0 ?  message = "Todo changed to incomplete success!" : message = "Todo Completed Success!"
        return {
            statusCode : 200,
            message
        }
